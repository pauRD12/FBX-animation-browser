...
class CreateGifs():
    ...
    def topNet(self):
        ...
        #Destroy all nodes aftet finishing script
        ps.parm("script").set('''
import hou
...
''')
        ...
        for file in os.listdir(BASE_DIR):
            ...
            if extension == ".fbx":
                #names_dict[count] = file.removesuffix(".fbx")
                count += 1
                ...
                #create a diferent camera for each fbx (wedge attr 2)
...
class Ui(QtWidgets.QWidget):
    ...
    def build_layout(self):
        ...
        # Crear el widget de contenido y su layout
        ...
        btn.clicked.connect(self.updateButton) #Signal del boton
    ...
    def loadGifs(self):
        ...
        for gif in os.listdir(BASE_DIR):
            if gif.endswith('.gif'):              
                boton = BotonAnimado(os.path.join(BASE_DIR, gif))
                boton.setIconSize(QtCore.QSize(200,200)) 
                boton.setText(gif.removesuffix(".gif")) # The text on the button will be the Fbx name.
                self.button_group.addButton(boton) #Add buttons to a group
                
                self.content_layout.addWidget(boton, v, h) 
                boton.clicked.connect(lambda checked=False, gif=gif: self.create_import_node(gif)) #Activate signal
    ...
    def create_import_node(self, gif):
        ...
        #Create node and add the name of the fbxfile
...
class BotonAnimado(QtWidgets.QToolButton):
